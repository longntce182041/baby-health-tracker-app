...
function DoctorAppointmentsPage() {
  ...
  const weekStart = useMemo(
    () => getWeekStart(new Date(selectedDate)),
    [selectedDate],
  );
  ...
  const handleSubmitRegister = async () => {
    try {
      ...
      const selectedDateObj = new Date(`${registerDate}T00:00:00`);
      const todayObj = new Date();
      todayObj.setHours(0, 0, 0, 0);

      if (selectedDateObj < todayObj) {
        setRegisterError("Không thể chọn ngày trong quá khứ");
        return;
      }
    ...
    <section className="dashboard-content appointments-page">
      ...
      <div className="appointments-top-grid">
        <section className="schedule-calendar-card">
          ...
          <div
            className={`schedule-date-grid ${viewMode === "month" ? "is-month" : ""}`.trim()}
          >
        ...
        <aside className="schedule-legend-card">
          ...
          <p className="legend-note">
            Booked slots are automatically locked and cannot be edited or
            deleted.
          </p>
        </aside>
      </div>
      ...
      <section id="daily-slot-section" className="time-slot-grid-card">
        ...
        {!loading && error ? (
          <p className="list-state list-error">{error}</p>
        ) : null}
            ...
            slots.map((slot) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`slot-pill ${isBooked ? "booked" : "available"}`.trim()}
                  >
                    {isBooked ? "Booked" : "Available"}
                  </span>
                  <p>{isBooked ? "Video" : "Video"}</p>
                  {isBooked && slot.patient_names?.[0] ? (
                    <strong>{slot.patient_names[0]}</strong>
                  ) : null}
                </article>
              );
            })}
      ...
      {isRegisterModalOpen ? (
        ...
        >
          <div className="schedule-modal">
            ...
            <label>
              ...
              <div className="date-input-wrapper">
                <input
                  ...
                  min={toDateKey(today)}
                  id="register-date-input"
                />
                <button
                  type="button"
                  className="calendar-icon-btn"
                  onClick={() =>
                    document.getElementById("register-date-input").showPicker()
                  }
            ...
            {registerError ? (
              <p className="list-state list-error">{registerError}</p>
            ) : null}

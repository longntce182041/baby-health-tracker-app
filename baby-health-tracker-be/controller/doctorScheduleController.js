...
const registerWeeklySchedule = async (req, res) => {
  ...
  try {
    ...
    for (const day of days) {
      ...
      const scheduleDate = new Date(day.date + "T00:00:00.000Z");
      if (Number.isNaN(scheduleDate.getTime())) {
        return res
          .status(400)
          .json({
            message: "Invalid date format in days. Expected YYYY-MM-DD",
          });
      }
...
const getDoctorSchedule = async (req, res) => {
  ...
  try {
    ...
    // Consolidate schedules into a single structure with normalized date format

...
const updateProfile = async (req, res) => {
  try {
    ...
    if (Object.keys(accountUpdates).length > 0) {
      updateTasks.push(
        accountService.updateAccountById(accountId, accountUpdates),
      );
    }
    ...
    if (
      ...
      updateTasks.push(
        parentService.updateParentById(account.parent_id, parentUpdates),
      );
    }
...
const updateWalletPoints = async (req, res) => {
  try {
    ...
    if (!action || !["add", "subtract"].includes(action)) {
      return res
        .status(400)
        .json({ message: "action must be add or subtract" });
    }
    ...
    let parent = null;
    if (action === "add") {
      parent = await parentService.addWalletPoints(
        account.parent_id,
        parsedPoints,
      );
    } else {
      parent = await parentService.deductWalletPoints(
        account.parent_id,
        parsedPoints,
      );
      if (!parent) {
...
const ViewTransactionHistory = async (req, res) => {
  try {
    ...
    const parent = await parentService.findParentById(req.user.parent_id);
    if (!parent) {
      return res.status(404).json({ message: "Parent not found" });
    }

    const transactionHistory = await parentService.viewTransactionHistory(
      req.user.parent_id,
    );
    return res.status(200).json({
  ...
  try {
    ...
    const data = parentAccounts.map((account) => {
      const parent = account.parent_id
        ? parentMap.get(account.parent_id.toString())
        : null;
...
const adminGetParentDetail = async (req, res) => {
  try {
    ...
    const account = await accountService.findAccountByIdAndRole(
      account_id,
      "parent",
    );
...
const adminUpdateParent = async (req, res) => {
  try {
    ...
    if (Object.keys(accountUpdates).length > 0) {
      updateTasks.push(
        accountService.updateAccountById(account._id, accountUpdates),
      );
    }
    ...
    const updatedAccount = await accountService.findAccountByIdAndRole(
      account_id,
      "parent",
    );
...
const adminChangeParentStatus = async (req, res) => {
  try {
    ...
    if (parsedStatus === null) {
      return res
        .status(400)
        .json({ message: "status must be boolean (true/false)" });
    }
    ...
    const updatedAccount = await accountService.updateAccountStatusById(
      account_id,
      parsedStatus,
    );
  ...
  try {
    ...
    const data = doctorAccounts.map((account) => {
      const doctor = account.doctor_id
        ? doctorMap.get(account.doctor_id.toString())
        : null;
...
const adminAddDoctor = async (req, res) => {
  try {
    ...
    if (normalizedExperience === null) {
      return res.status(400).json({
        message:
          "experience must be an array of objects containing certificate_name",
      });
    }
...
const adminUpdateDoctor = async (req, res) => {
  try {
    ...
    if (account.doctor_id && Object.keys(doctorUpdates).length > 0) {
      updateTasks.push(
        doctorService.updateDoctor(account.doctor_id, doctorUpdates),
      );
    }

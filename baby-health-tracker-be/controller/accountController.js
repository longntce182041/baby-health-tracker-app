...
const register = async (req, res) => {
    ...
    try {
        ...
        const accountData = { email, password: hashedPassword, parent_id: newParent._id, role: 'parent' };
        ...
        res.status(201).json({ message: 'Account created successfully. OTP sent to email.' });
    } catch (error) {
...
const verifyOtp = async (req, res) => {
    ...
    try {
        const record = await EmailOtp.findOne({ email, otp, purpose: 'register', used: false });
        if (!record) {
...
const resetPassword = async (req, res) => {
    ...
    try {
        ...
        const record = await EmailOtp.findOne({ email, otp, purpose: 'reset', used: false });
        if (!record) {
            return res.status(400).json({ message: 'Invalid OTP' });
        }

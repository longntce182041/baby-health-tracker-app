...
const addBaby = async (req, res) => {
  const { full_name, avatar_url, day_of_birth, gender, alergy, note } =
    req.body;
  ...
  try {
    ...
    if (note !== undefined && normalizedNote === null) {
      return res.status(400).json({
        message:
          "note must be a string, object { content }, or an array of these",
      });
    }
    ...
    const baby = await babyService.createBaby({
      ...
      day_of_birth,
      gender,
      alergy: Array.isArray(alergy) ? alergy : alergy ? [alergy] : [],
      note: normalizedNote || [],
      status: true,
    });
...
const updateBaby = async (req, res) => {
  ...
  try {
    ...
    if (alergy !== undefined) {
      updateData.alergy = Array.isArray(alergy) ? alergy : [alergy];
    }

    if (Object.keys(updateData).length === 0) {
      return res.status(400).json({
        message:
          "At least one of full_name, avatar_url, day_of_birth, gender, note, alergy is required",
      });
    }
    ...
    res
      .status(200)
      .json({ message: "Baby updated successfully", data: updated });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
};
...
const shareBabyToParent = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Baby shared successfully", data: updated });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
};
...
const addBabyNote = async (req, res) => {
  ...
  try {
    ...
    if (!isOwner) {
      return res
        .status(403)
        .json({ message: "Not allowed to add note for this baby" });
    }
    ...
    res
      .status(200)
      .json({ message: "Baby note added successfully", data: updated });
  } catch (error) {
    res.status(500).json({ message: "Server error", error: error.message });
  }
};

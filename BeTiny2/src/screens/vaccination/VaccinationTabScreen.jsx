...
import { getVaccinationSchedules } from "../../api/vaccinationApi";
...
const MOCK_UPCOMING = [
  {
    ...
    day: 10,
    monthAbbr: "TH09",
    name: "6 trong 1 (Mũi 1)",
    desc: "Bạch hầu, Ho gà, Uốn ván, Bại liệt, Viêm gan B...",
    warning: true,
  },
  {
    id: "2",
    day: 25,
    monthAbbr: "TH10",
    name: "Phế cầu khuẩn (Mũi 2)",
    desc: "Phòng các bệnh do phế cầu như viêm phổi, viêm màng não.",
    warning: false,
  },
  {
    id: "3",
    ...
    desc: "Phòng bệnh sởi, quai bị, rubella.",
    warning: false,
  },
];
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        ...
        console.log("VaccinationTab getVaccinationSchedules response:", res);

        const data =
          res?.data?.data ?? (Array.isArray(res?.data) ? res.data : []);
        console.log("VaccinationTab vaccination schedules data:", data);

        if (data.length > 0) {
          // Filter for upcoming/scheduled vaccinations and sort by date
          const upcomingSchedules = data
            .filter((v) => v.status === "scheduled" || !v.status)
            .sort(
              (a, b) => new Date(a.injection_date) - new Date(b.injection_date),
            )
            .slice(0, 5);

          const mapped = upcomingSchedules.map((v, i) => {
            const vaccDate = v.injection_date || v.scheduled_date;
            const date = vaccDate ? new Date(vaccDate) : new Date();

            return {
              id: v._id || v.id || String(i),
              day: date.getDate(),
              monthAbbr: `TH${String(date.getMonth() + 1).padStart(2, "0")}`,
              name: v.vaccine_name || "Tiêm chủng",
              desc:
                v.notes && v.notes.length > 0
                  ? v.notes[0].content
                  : v.location?.branch_name || "",
              warning: false, // Can be enhanced based on date proximity
            };
          });

          setUpcoming(mapped.length > 0 ? mapped : MOCK_UPCOMING);
        } else {
          console.log("VaccinationTab: No schedules found, using mock data");
          setUpcoming(MOCK_UPCOMING);
        }
      } catch (err) {
        console.error("VaccinationTab getVaccinationSchedules error:", err);
        setUpcoming(MOCK_UPCOMING);
      }
    };
    fetch();
        ...
        {!hasBabies ? (
          <View style={styles.emptyBlock}>
            ...
            <Text style={styles.hint}>
              Thêm bé ở mục Sức khỏe hoặc Trang chủ để xem lịch tiêm chủng.
            </Text>
            <TouchableOpacity
              style={styles.btn}
          ...
          <>
            ...
            >
              <View style={styles.babyAvatarWrap}>
                <View style={styles.babyImg}>
                  {(baby?.avt ?? baby?.avatar ?? baby?.avatar_url) ? (
                    ...
                    <Text style={styles.babyInitial}>
                      {getInitials(baby?.full_name)}
                    </Text>
                  )}
                </View>
              </View>
              ...
              <Text style={styles.babyDob}>
                Sinh ngày: {formatDob(baby?.date_of_birth)}
              </Text>
            </TouchableOpacity>
            ...
            <View style={styles.beforeAfterWrap}>
              ...
              >
                <Ionicons
                  ...
                  size={20}
                  color={colors.pinkAccent}
                  style={styles.beforeAfterIcon}
                />
                <Text style={styles.beforeAfterTitle}>
                  Những điều cần trước và sau tiêm
                </Text>
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.timeline}>
              ...
              {upcoming.map((v) => (
                <View
                  ...
                  style={[
                    styles.vaccineCard,
                    v.warning && styles.vaccineCardWarning,
                  ]}
                >

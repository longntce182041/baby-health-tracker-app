...
import { getBabyById, getBabies } from "../../api/babyApi";
...
export default function VaccinationBookConfirmScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoadingBaby(true);
        const res = await getBabyById(resolvedBabyId);
  ...
  const handleConfirm = async () => {
    ...
    if (!resolvedBabyId || !vaccineId || !centerId) {
      Alert.alert(
        "Lỗi",
        "Thiếu thông tin bé hoặc vắc xin, không thể đặt lịch.",
      );
      return;
    }
    try {
      ...
      const isOk = !!res?.data && !res?.error;
      if (!isOk) {
        Alert.alert(
          "Lỗi",
          res?.message || "Không thể đặt lịch tiêm, vui lòng thử lại.",
        );
        setSubmitting(false);
        return;
      }
      ...
      Alert.alert(
        "Lỗi",
        e?.message || "Không thể đặt lịch tiêm, vui lòng thử lại sau.",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <View style={styles.container}>
      ...
      >
          ...
          <View style={styles.timeSlots}>
            {TIME_SLOTS.map((t) => (
              <TouchableOpacity
                ...
                style={[
                  styles.timeItem,
                  selectedTime === t && styles.timeItemSelected,
                ]}
              ...
              >
                <Text
                  style={[
                    styles.timeItemText,
                    selectedTime === t && styles.timeItemTextSelected,
                  ]}
                >
                  {t}
                </Text>
        ...
        <TouchableOpacity
          style={[styles.btnConfirm, submitting && styles.btnConfirmDisabled]}

...
export default function HealthLogAddScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    if (editMoment._original) {
      if (editMoment._original.tempurature)
    ...
    if (editMoment.photos && Array.isArray(editMoment.photos)) {
      setPhotos(editMoment.photos);
    }
  }, [editMoment]);
  ...
  const handleSave = async () => {
    if (isEdit && editMoment) {
      try {
        if (!babyId || !editMoment.id) {
          navigation.goBack();
          return;
        }

        // Map frontend fields to backend format for update
        const payload = {
          tempurature: temperature
            ? parseFloat(temperature)
            : editMoment._original?.tempurature || 36.5,
          sleep: sleepHours
            ? parseFloat(sleepHours)
            : editMoment._original?.sleep || 8,
          image_urls:
            photos.length > 0 ? photos : editMoment._original?.image_urls || [],
        ...
        console.log("Updating health log with payload:", payload);
        const res = await updateHealthLog(editMoment.id, payload);
        ...
        console.error("updateHealthLog error:", e?.message || e);
        navigation.goBack();
      }
    } else {
      try {
        ...
        // Map frontend fields to backend format for create
        const payload = {
          tempurature: temperature ? parseFloat(temperature) : 36.5,
          sleep: sleepHours ? parseFloat(sleepHours) : 8,
        ...
        console.log("Creating health log with payload:", payload);
        ...
        console.error("createHealthLog error:", e?.message || e);
        navigation.goBack();
      }
    }
  };
      ...
      >
        <View style={styles.headerRow}>
          ...
          <Text style={styles.headerTitle}>
            {isEdit ? "Cập nhật nhật ký" : "Thêm nhật ký"}
          </Text>
                ...
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    isActive && [
                      ...
                      {
                        backgroundColor: BABY_STATUS_BG[s.key],
                        borderColor: color,
                      },
                    ],
                  ]}
            ...
            return (
              ...
              >
                <View
                  style={[
                    styles.symptomCheck,
                    checked && styles.symptomCheckActive,
                  ]}
                >
                  {checked && (

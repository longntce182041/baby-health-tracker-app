...
function getCardBorderColor(m) {
  if (m.babyStatus && BABY_STATUS_COLORS[m.babyStatus])
    return BABY_STATUS_COLORS[m.babyStatus];
    ...
    try {
      ...
      if (Array.isArray(res?.data)) {
        ...
        const transformedLogs = res.data.map((log) => {
          // Format time from log_date
          const date = new Date(log.log_date);
          const hours = date.getHours();
          const minutes = date.getMinutes();
          const period = hours >= 12 ? "Chiều" : "Sáng";
          const displayHours = hours > 12 ? hours - 12 : hours || 12;
          const time = `${String(displayHours).padStart(2, "0")}:${String(minutes).padStart(2, "0")} ${period}`;

          // Build health array from backend fields
          const health = [];
            ...
            type:
              health.length > 0
                ...
                : log.image_urls?.length > 0
                  ...
                  : "default",
            time,
            title: log.notes || "Nhật ký sức khỏe",
            desc: log.notes || "",
            babyStatus: log.mood || "normal",
            symptoms: Array.isArray(log.symptoms) ? log.symptoms : [],
            photos: Array.isArray(log.image_urls) ? log.image_urls : [],
        ...
        // Group by date
        const grouped = transformedLogs.reduce((acc, log) => {
          const date = new Date(log.log_date);
          ...
          if (date.toDateString() === today.toDateString()) {
            dateLabel = `Hôm nay, ${date.toLocaleDateString("vi-VN")}`;
          } else if (date.toDateString() === yesterday.toDateString()) {
            dateLabel = `Hôm qua, ${date.toLocaleDateString("vi-VN")}`;
          } else {
            dateLabel = date.toLocaleDateString("vi-VN");
          }

          const existing = acc.find((g) => g.dateLabel === dateLabel);
          if (existing) {
            existing.moments.push(log);
          } else {
            acc.push({ dateLabel, moments: [log] });
          }
          return acc;
        }, []);
        ...
        {momentGroups.map((group, gIdx) => (
          <View key={gIdx} style={styles.momentGroup}>
            ...
            {group.moments.map((m) => {
              ...
              const photosArray = Array.isArray(m.photos)

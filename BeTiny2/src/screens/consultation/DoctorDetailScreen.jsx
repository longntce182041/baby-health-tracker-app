...
export default function DoctorDetailScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setSchedules(
          scheduleFromApi.length > 0 ? scheduleFromApi : scheduleNested,
        );
      } catch (e) {
        console.error("DoctorDetailScreen fetch error:", e);
        if (doctorParam) {
          setDoctor(doctorParam);
          setSchedules(
            Array.isArray(doctorParam.schedules) ? doctorParam.schedules : [],
          );
        } else {
          setDoctor(null);
          setSchedules([]);
  ...
  const initial =
    doctor.full_name && doctor.full_name.trim()
    ...
    <View style={styles.container}>
          ...
          <View style={styles.avatarContainer}>
            ...
            <View style={styles.ratingChip}>
              {ratingValue != null && ratingValue !== "" && (
                <Ionicons name="star" size={12} color="#745E00" />
              )}
              <Text
                style={[
                  styles.ratingChipText,
                  ratingValue == null || ratingValue === ""
                    ? styles.ratingChipTextMuted
                    : null,
                ]}
              >
                {ratingDisplay}
      ...
      <View style={[styles.bookingBar, { paddingBottom: insets.bottom + 20 }]}>
        <View style={styles.feeWrap}>
          ...
          <Text
            style={[styles.feeWrapValue, !hasFee && styles.feeWrapValueMuted]}
          >
        ...
        >
          <Text
            style={[styles.btnMainText, !canBook && styles.btnMainTextDisabled]}
          >
            Đặt lịch ngay
          </Text>
        </TouchableOpacity>

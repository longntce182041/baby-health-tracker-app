...
export const getDoctors = async (params = {}) => {
  try {
    ...
    console.warn('getDoctors error, dùng dữ liệu mock:', error?.message || error);
...
export const getDoctorSchedules = async (doctorId, params = {}) => {
  try {
    ...
    console.warn('getDoctorSchedules error, dùng mock:', error?.message || error);
    const doctor = getMockDoctorById(doctorId);
    const list = Array.isArray(doctor?.schedules) ? doctor.schedules : MOCK_DOCTOR_SCHEDULES;
